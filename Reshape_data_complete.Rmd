...
pines_long <- Loblolly %>% filter(Seed  %in% seeds)
...
```{r}
pines_wide <- pivot_wider(pines_long, 
                      names_from = Seed, #column names from Seed column
                      values_from = height) #values column from height
pines_wide
```
...
```{r}
mutate(pines_wide, T301_301 = `301` - `303`)
```
